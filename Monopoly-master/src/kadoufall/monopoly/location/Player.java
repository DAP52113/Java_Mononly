...
public class Player extends Location {
	public static final int CASH_NUMBER = 10000;
	public static final int DEPOSIT_INIT = 10000;
	public static final int COUPON_INIT = 50;
	public static final int HOUSE_NUM_INIT = 0;
	...
	// Add players to a point when creating a new player
	public Player(String name, int playerID, Point point, String icon, String largeIcon) {
		super(name, point);
		addCash(CASH_NUMBER);
		addDeposit(DEPOSIT_INIT);
		addCoupon(COUPON_INIT);
		addHouseNum(HOUSE_NUM_INIT);
	...
	public double getCash() {
		...
		double re = bg.setScale(NEW_SCALE, BigDecimal.ROUND_HALF_UP).doubleValue();
		return re;
	}
	...
	public void step(ArrayList<Point> points, ArrayList<ArrayList<Location>> streets, ArrayList<Player> players) {
		...
		if (barricade != null) {
			...
			alert.setContentText("Hit a roadblock =_=");
			alert.showAndWait();
